...
/**
 * motaphoto functions and definitions
 *
...
function motaphoto_setup()
{
	/*
		...
		* If you're building a theme based on motaphoto, use a find and replace
		* to change 'motaphoto' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'header' => esc_html__('Primary', 'motaphoto'),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'motaphoto_custom_background_args',
			array(
 ...
 */
function motaphoto_content_width()
{
	$GLOBALS['content_width'] = apply_filters('motaphoto_content_width', 640);
}
add_action('after_setup_theme', 'motaphoto_content_width', 0);
...
function motaphoto_widgets_init()
{
	register_sidebar(
		array(
			'name'          => esc_html__('Sidebar', 'motaphoto'),
			'id'            => 'sidebar-1',
			'description'   => esc_html__('Add widgets here.', 'motaphoto'),
